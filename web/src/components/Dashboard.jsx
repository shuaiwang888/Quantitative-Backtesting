...
/**
 * 把 iwencai 行转成弹窗 target（已移到 utils/chart.js 复用）。
 */
...
export default function Dashboard({ onError, onStatus }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      onStatus?.(
        `首页已更新：${market?.datas?.length || 0} 指数 · ${watch?.items?.length || 0} 自选股`
      );
    } catch (e) {
      onError?.(e);
    } finally {
  ...
  // 整合后的最新缓存时间（取两者最新）
  const latestTs = Math.max(marketCache.ts, watchlistCache.ts);
